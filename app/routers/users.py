...
async def get_user(user_id: uuid.UUID, db: Annotated[AsyncSession, Depends(database.get_db)]):
...
async def get_user_posts(user_id: uuid.UUID, db: Annotated[AsyncSession, Depends(database.get_db)]):
...
async def update_user(user_id: uuid.UUID, user_update: UserUpdate, current_user: CurrentUser, db: Annotated[AsyncSession, Depends(database.get_db)]):
    ...
    if user_update.email is not None and user_update.email.lower() != user.email.lower():
        result = await db.execute(select(models.User).where(func.lower(models.User.email) == UserCreate.email.lower()))
        existing_email = result.scalars().first()
        if existing_email:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email already exists")
...
async def delete_user(user_id: uuid.UUID, current_user:CurrentUser, db: Annotated[AsyncSession, Depends(database.get_db)]):
